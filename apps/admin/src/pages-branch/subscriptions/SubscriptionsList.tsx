...
type ResponseShape = {
  ...
  stats: { total: number; active: number; expired: number; depleted: number; paid: number; revenue: number }
}
    ...
    <div className="page-container">
      ...
      <div className="page-content">
        <div className="page-content-inner">
          ...
          <Card style={{ marginBottom: 24 }}>
            <Space wrap size="middle" style={{ width: '100%', justifyContent: 'space-between' }}>
              <Space wrap>
                ...
                <Select allowClear size="large" style={{ width: 170 }} placeholder="حالة الاشتراك" value={status} onChange={setStatus} options={[{ value: 'active', label: 'نشط' }, { value: 'expired', label: 'منتهي' }, { value: 'depleted', label: 'مستهلك' }, { value: 'cancelled', label: 'ملغي' }]} />
                <Select allowClear size="large" style={{ width: 170 }} placeholder="حالة الدفع" value={paymentStatus} onChange={setPaymentStatus} options={[{ value: 'completed', label: 'مدفوع' }, { value: 'pending', label: 'بانتظار الدفع' }, { value: 'failed', label: 'فشل الدفع' }]} />
          ...
          <Card>
            <Table<SubscriptionRow> rowKey="id" loading={loading} dataSource={rows} columns={columns as any} locale={{ emptyText: <Empty description="لا توجد اشتراكات حالياً" /> }} pagination={{ pageSize: 10, showSizeChanger: true }} scroll={{ x: 900 }} />
          </Card>
        </div>
      </div>
    </div>
