...
export default function Overview() {
  ...
  const loadOverview = async () => {
    ...
    try {
      ...
      const data = await apiGet(`/reports/overview?${params.toString()}`)
      setOverview(data)
    } catch (error) {
  ...
  const loadRecentBookings = async () => {
    ...
    try {
      const data = await apiGet(`/bookings/branch/me?page=1&limit=5`)
          ...
          <Row gutter={[24, 24]}>
            ...
            <Col xs={24} lg={12}>
              <Card 
                ...
                extra={<Button type="link" onClick={() => window.location.href = '/bookings'}>
